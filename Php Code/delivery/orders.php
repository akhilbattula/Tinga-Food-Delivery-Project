...
if(isset($_POST['d_id'])){
    ...
    $did = mysqli_real_escape_string($conn,htmlspecialchars($_POST['d_id']));
    
    $query = "SELECT * FROM `tbl_order` WHERE `delivery_id`='$did' ORDER BY `created_on` DESC";
        ...
        if($count === 0){
            ...
            while($row = mysqli_fetch_array($result)){
                  ...
                  $bean  = array('order_id'=> $row['order_id'],
                                  ...
                                  'customer_name'=>$name,
                                  'customer_id'=>$id,
                                  'customer_phone_number'=>$row_customer['phone_number'],
